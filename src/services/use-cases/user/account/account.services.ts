...
import { BadRequestsException, DoesNotExistsException, TooManyRequestsException } from "../exceptions";
...
export class AccountServices {
  ...
  async createTransactionPin(userId: string, pin: string) {
    try {
      ...
      await this.dataServices.users.update({ _id: user?._id }, { transactionPin: hashedPin })
  ...
  async updateTransactionPin(payload: { userId: string, pin: string, oldPin: string }) {
    try {
      const { userId, pin, oldPin } = payload
      const user = await this.dataServices.users.findOne({ _id: userId })
      if (!user) throw new DoesNotExistsException('user does not exists')
      const comparePin = await compareHash(oldPin, user?.transactionPin)
      if (!comparePin) throw new BadRequestsException('old transaction pin is invalid')
      const hashedPin = await hash(pin)
      await this.dataServices.users.update({ _id: user?._id }, { transactionPin: hashedPin })
  ...
  async kyc(payload: { userType: string, phone: string, userId: string, code: string }) {
    try {
      ...
      const codeSentInPast24H = await this.inMemoryServices.get(phoneCodeCountKey)
      if (codeSentInPast24H) {
        ...
        throw new TooManyRequestsException(`Phone was recently updated. Try again in ${nextTryOpening}`)
      }

      // for mobile users only

      const codeSent = await this.inMemoryServices.get(phoneVerificationCodeRedisKey);
      if (!code) {

        if (codeSent) {
          const codeExpiry = await this.inMemoryServices.ttl(phoneVerificationCodeRedisKey) as Number || 0;
          return {
            status: 202,
  ...
  async uploadIdCard(payload: { documentType: string, url: string, userId: string }) {
    try {
      ...
      await this.dataServices.users.update({ _id: user?._id }, { $set: { document } })
      return {
        status: 200,
