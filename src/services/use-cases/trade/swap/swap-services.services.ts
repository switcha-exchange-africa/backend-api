...
import { BadRequestsException, DoesNotExistsException } from "./../../user/exceptions";
...
export class SwapServices {
  ...
  async swap(body: SwapDto, userId: string) {
    ...
    const { value: value1 } = await this.http.get(url1, config);
    const { value: value2 } = await this.http.get(url2, config);
